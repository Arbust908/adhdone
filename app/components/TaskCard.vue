<template>
  <div
    ...
    :class="{ 'opacity-60': task.finishedAt }" @click="$emit('edit', task)">
    <div class="flex items-start gap-3">
      <!-- Checkbox -->
      <button class="mt-0.5 shrink-0 w-5 h-5 rounded border-2 transition-colors flex items-center justify-center"
        :class="task.finishedAt
          ? 'bg-green-500 border-green-500 text-white'
          : 'border-gray-300 dark:border-gray-600 hover:border-green-400'" @click.stop="$emit('toggle', task)"
        :aria-label="task.finishedAt ? 'Mark as pending' : 'Mark as finished'">
        <span v-if="task.finishedAt" class="text-sm">✓</span>
      </button>
      ...
      <div class="flex-1 min-w-0">
        ...
        <p v-if="task.description" class="text-md text-gray-500 dark:text-gray-400 mt-1 line-clamp-2">
          {{ task.description }}
        </p>
        ...
        <div class="flex flex-wrap items-center gap-1.5 mt-2">
          <!-- Recurring badge -->
          <UBadge v-if="task.recurringCron" color="secondary" variant="soft" size="xs">
            🔄 Recurring
          </UBadge>

          <!-- Deadline badge -->
          <UBadge v-if="task.deadline && !task.finishedAt" :color="deadlineColor" variant="soft" size="xs">
            {{ deadlineLabel }}
          </UBadge>

          <!-- Reminder badge -->
          <UBadge v-if="hasReminders && !task.finishedAt" color="warning" variant="soft" size="xs">
...
const deadlineColor = computed(() => {
  if (!props.task.deadline) return 'info';
  ...
  if (diffHours < 6) return 'warning';   // soon (< 6h)
